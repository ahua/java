 ...
 */
class BrokerImpl extends Broker {
    
    /** The manager */
    private Manager manager;
    
    /**
     * Create a broker for the given path to use the given manager.
     * @param manager the manager
     * @param the path to the resource
     * @throws IOException if an error occurs
     */
    BrokerImpl(Manager manager, Path path) throws IOException {
    ...
    public Broker createFile(String name) {
        try {
            ...
            return (created != null)
                    ? new BrokerImpl(manager, created)
                    : null;
        } catch (IOException e) {
            return null;
        }
